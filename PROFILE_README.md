...
| [cloud-security-posture](https://github.com/you/cloud-security-posture) | OPA/Rego policies, CIS benchmark automation, findings reports |

## 🛠 Skills

`Terraform` `GitHub Actions` `AWS Security` `IAM` `KMS` `OPA/Rego`  
`SAST (Semgrep/Bandit)` `DAST (OWASP ZAP)` `Trivy` `Syft (SBOM)` `gitleaks`  
`Python` `Bash` `Linux` `Docker` `CIS Benchmarks` `NIST CSF`
